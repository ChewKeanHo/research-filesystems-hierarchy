...
user-specific, non-user (e.g. daemon server) programs and applications
personalized for `root` account.

Generally, you **SHOULD** place `root` account own custom programs and
applications here.

All files here are available only to the `root` account but full path calling
is required.
