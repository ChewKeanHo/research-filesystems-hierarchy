...
This is the base directory for only critical libraries used by critical
...
simplifying the package managements to target `/usr/lib` only.
...
early booting stage without conflicting with existing POSIX compliant programs.
In the case of `/lib` being symbolic linked to `/usr/lib`, you **SHOULD NOT**
place anything here and instead use `/usr/lib` exclusively.
