...
partitions, or volume managed partitions for some operating systems (OS) notably
Red Hat, and Fedora. Otherwise, `/media` is used instead.
...
`/var/media` caters for one or more devices. This made `/mnt` a temporary
mounting directory only for sysadmins and root user to debug mountable
transactions.
